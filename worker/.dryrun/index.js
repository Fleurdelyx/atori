...
app.get("/api/health", (c) => c.json({ ok: true, service: "atori-cloud", version: 1 }));
app.get("/api/manifest", async (c) => {
  const obj = await c.env.LIBRARY.get(MANIFEST_KEY);
...
app.put("/api/manifest", async (c) => {
  ...
  await c.env.LIBRARY.put(MANIFEST_KEY, body, {
    httpMetadata: { contentType: "application/json" }
  });
  return c.json({ ok: true });
});
app.get("/api/stream/*", async (c) => {
  const key = keyAfter(c.req.path, "/api/stream/");
